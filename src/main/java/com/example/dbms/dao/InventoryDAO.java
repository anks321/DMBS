...
public class InventoryDAO {
	...
	public void save(Inventory inventory) {
		String sql = "insert into inventory(item_id, cost, quantity, expiry_date, name, mess_id, section_id) values (?,?,?,?,?,?,?);";
		temp.update(sql, inventory.getItem_id(), inventory.getCost(), inventory.getQuantity(),
				inventory.getExpiry_date(), inventory.getName(), inventory.getMess_id(), inventory.getSection_id());
	}

	public void update(int cost, int quantity, String name, int item_id) {
		String sql = "update inventory set cost = ?, quantity = ?, name = ? where item_id = ?";
